class Carrousel {
  ...
  createControlEls() {
    ...
    this.carrouselEl.append(
      (this.arrowLeft = this.createControlEl(
        "control icon arrow-left",
        this.icons.arrowLeft
      )),
      (this.arrowRight = this.createControlEl(
        "control icon arrow-right",
        this.icons.arrowRight
      )),
      (this.bulletsEl = this.createControlEl("bullets")),
      (this.fullViewBtn = this.createControlEl(
        "control icon full-view-btn",
        this.icons.fullViewMax
      )),
      (this.playpauseBtn = this.createControlEl(
        "control icon auto-play-btn",
        this.icons.slidePause
      )),
      (this.constrolsHideBtn = this.createControlEl(
        "icon controls-hide-btn",
        this.icons.controlsHide
      ))
    );
  }
  ...
  initSlides() {
    ...
    for (let i = 0; i < this.slides.length; i++) {
      ...
      this.slides[
        i
      ].style.backgroundImage = `var(--backgroundOverlay), url(${this.slides[i].src})`;
      //bullets
      let bulletEl = this.createControlEl(
        "icon bullet",
        i === this.currentSlideIndex
          ? this.icons.bulletActive
          : this.icons.bulletInactive
      );
  ...
  createEventListeners() {
    ...
    this.arrowLeft.addEventListener("click", () =>
      this.slideTo(this.currentSlideIndex - 1, true)
    );
    ...
    this.constrolsHideBtn.addEventListener("click", () =>
      this.toggleHideControls()
    );
    for (let i = 0; i < this.slides.length; i++) {
      this.slides[i].addEventListener("dblclick", () =>
        this.toggleFullPageView()
      );
    ...
    this.carrouselEl.addEventListener("keydown", (e) => {
      ...
      else if (Number(e.key) > 0 && Number(e.key) <= this.slides.length)
        this.slideTo(Number(e.key) - 1, true);
      else if (e.key.toLowerCase() === "c") this.toggleHideControls();
    });

    //fullscreen change event
    this.carrouselEl.addEventListener("fullscreenchange", () =>
      this.toggleFullViewBtn(!window.screenTop && !window.screenY)
    );
  }

  toggleHideControls() {
    this.carrouselEl.classList.toggle("hide-controls");
    if (this.carrouselEl.classList.contains("hide-controls"))
      this.constrolsHideBtn.innerHTML = this.icons.controlsHide;
  ...
  toggleFullPageView() {
    if (!this.fullPageState) {
      if (this.carrouselEl.requestFullscreen)
        this.carrouselEl.requestFullscreen();
      else if (this.carrouselEl.webkitRequestFullscreen)
        this.carrouselEl.webkitRequestFullscreen(); /* Safari */
      else if (this.carrouselEl.msRequestFullscreen)
        this.carrouselEl.msRequestFullscreen(); /* IE11 */
    } else {
